...
export const resetUserPassword = async (req, res, next) => {
  ...
  try {
    ...
    const hashedResetToken = crypto
      ...
      .update(resetToken)
      .digest("hex");
    const user = findUserForPasswordResetRepo(hashedResetToken);
    if (!user) {
      return next(
        new ErrorHandler(401, "user not found! register yourself now!!")
      );
    ...
    if (password !== confirmPassword) {
      return next(
        new ErrorHandler(400, "mismatch new password and confirm password!")
      );
    }
...
export const updateUserProfileAndRole = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ success: true, msg: "user role updated successfully" });
  } catch (error) {
    return next(new ErrorHandler(400, error));
  }
};
